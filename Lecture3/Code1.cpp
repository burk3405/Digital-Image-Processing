...
if(argc>3 || argc<3)
{
cout<<"Usage: "<<argv[0]<<" inputFile.bmp outputFile.bmp"<<endl;
return 1;
}
...
if(image.writeHeader(output)!=true)
{
...
if(image.writeInfoHeader(output)!=true)
{
cout<<"Error writing information header to output file"<<endl;
input.close();
output.close();
return 1;
}
image.readBitmap(input);
image.gray();
